...
影响范围	  影响部分或整个渲染树	      只影响元素外观
性能消耗   高（需要重新计算布局）	      低（只需重新绘制）
触发条件	  几何属性变化                  外观属性变化
...
- 事件委托
- 使用CSS优化
- 避免table布局：table需要多次计算才能确定其元素在渲染树中的属性
...
当需要GPU重新排列布局时就是回流（重排）。
重排是浏览器必须重新计算页面布局的过程，而重绘只是将已经计算好的布局重新绘制到屏幕上。
优化时应优先减少重排，因为它的性能代价更高。
